...
import { lotMockData as lots, IAuction } from '@auction-app/models';
...
import { auctionUpcomingStyle } from '../theme/theme.ts';

const useStyles = createStyles(
  (theme): Record<string, CSSObject> => auctionUpcomingStyle(theme),
);
...
export function AuctionUpcoming({
  auction,
  registered = false,
  pickedUnits = ['2012', '1510'],
}: {
  auction: IAuction;
  registered: boolean;
  step: number;
  ...
  const pickedUnitsSelection: { [key: number]: boolean } = {};
  ...
  const Marker = ({ selected }: { selected: boolean }): JSX.Element => {
    return (
      <div
        style={{
          width: '35px',
  ...
  const ImagesModal = floorPlans.map((image: string) => (
            ...
            >
              ...
              >
                <Marker
                  ...
                  // id={auction.id}
                  // icon={pinAsset}
                  selected={Object.keys(rowSelection).includes(auction.id)}
                />
              </GoogleMapReact>
            </Card>
          </Grid.Col>
